...
### 이 방식에서 생기는 리스크 & 보완 원칙
  ...
  리스크 보완 원칙  
  ----------------- -----------------------------------------------------
  누가 무엇을       리소스는 **Terraform apply/destroy로만** 생성·삭제
  만들고 지웠는지   (콘솔에서 임의 생성·삭제 금지 원칙), 커밋/PR로 변경
  추적 어려움       이력 추적

  실수로 고비용     Budget Alert + 주간 Cost Explorer 점검 (12장 참고)
  리소스가 켜진 채  
  방치될 가능성     

  계정 정보 유출 시 콘솔 로그인 정보는 필요 인원(인프라팀)만 보유, 타
  전체 리소스 노출  팀에는 접속 정보 대신 **엔드포인트/접속 URL 등
                    결과물만 전달** (11장 참고)

  고비용 리소스를   10장 "리소스 생성 규칙" 참고 --- 사전 협의 후 생성
  아무나 생성 가능  원칙
  -----------------------------------------------------------------------
...
## 4. 확정된 아키텍처 요소
  ...
  비용 영향                                         
  --------------- --------------------------------- ---------------------
  **GPU**         GPU Node 미사용, AI도 CPU Pod로   기존 GPU 비용 제거
                  운영                              

  **Worker**      FE `t3.small × 2`, BE/WAS         작은 Node 다중 구성
                  `t3.large × N`                    

  **DB**          **AWS RDS PostgreSQL, Multi-AZ,   KT Cloud DB 제거, RDS
                  50GB**                            비용 발생

  **Vector DB**   PostgreSQL에 `pgvector` 적용      별도 Vector DB 불필요

  **Object        **Amazon S3**                     KT Cloud Object
  Storage**                                         Storage 제거
  ...
  **NAT**         `t3a.micro` NAT Instance          NAT Gateway 대비 비용
                                                    절감
  -----------------------------------------------------------------------
...
충분합니다.
  ...
  평가                                        
  ------------- -------- -------------------- -------------------------------------------
  t4g.nano /    2 vCPU / 약                   가장 저렴하지만 0.5GiB는 다수 팀원이 동시에
  t3.nano       0.5GiB   \$0.0052\~0.0065/h   이미지 pull·apt·terraform provider 다운로드
                                              등을 할 때 커넥션 트래킹(conntrack) 여유가
                                              부족할 수 있음

  **t3a.micro   2 vCPU / **\$0.0117/h         나노 대비 메모리 2배로 다수 동시 연결에
  (선정)**      1GiB     (확인됨)**           여유, AMD 기반이라 t3 대비 저렴, 버스터블
                                              크레딧으로 순간 트래픽도 대응 가능

  t3.micro      2 vCPU / 약 \$0.013/h (추정)  t3a.micro와 스펙 동일하나 소폭 더 비쌈
                1GiB                          

  t4g.micro     2 vCPU / 약 \$0.0104/h (추정) t3a.micro와 유사하게 저렴하나, ARM이라 NAT
                1GiB                          Instance AMI(주로 x86 기반 배포판) 호환성을
                                              별도 확인해야 함
  ---------------------------------------------------------------------------------------
...
### 5.2 NAT Instance 비용
  ...
  가정)                                                       
  -------------------- -------------------------------------- ------------
  t3a.micro 인스턴스   \$0.0117 × 730h ≈ **\$8.54/월**        \$0.0117 ×
                                                              368h ≈
                                                              **\$4.31**
  ...
  데이터               NAT Gateway의 GB당 처리                좌동
  전송(아웃바운드)     요금(\$0.045/GB) 없음. 일반 EC2 인터넷 
                       아웃바운드 요금만 적용(월 100GB까지    
                       무료 구간 존재)                        
  ------------------------------------------------------------------------
...
단, 기존 문서처럼 전체 월 비용에 `368/720`을 적용하지 않는다.
  ...
  Control Plane      EKS                                Cluster 유지시간
                                                        기준

  관리형 서비스      RDS, ElastiCache, OpenSearch       실제 리소스
                                                        유지기간 기준

  Storage            S3, EBS, ECR                       저장량 및 보유기간
                                                        기준
...
## 7. 일정별 비용 관리 계획
  ...
  포인트                         
  ------------------- ---------- ----------------------------------------
  **9/7\~9/13**       핵심       VPC/EKS/NodeGroup/NAT/ECR/RDS 등 구축
                      인프라     
                      구축       

  **9/14\~9/20**      서비스     FE/BE/AI CPU 워크로드 배포
                      이관       

  **9/21\~9/27**      통합       전체 서비스 운영, 실제 CPU/Memory 측정
                      테스트     

  **9/28\~10/6**      Final      리허설/발표에 필요한 리소스 유지, 종료
                                 후 정리
  -----------------------------------------------------------------------
...
## 8. Redis(ElastiCache) 처리 방안
  ...
  **OpenSearch**          `t3.small.search ×1`   **\$42.27**    OpenSearch Pod on
                                                                EKS
...
## 13. 미확정 / 확인 필요 항목 (Action Items)
  ...
  대상                                                            
  ---------------------------- ---------------------------------- ----------------------
  Redis 실제                   ElastiCache 비용은 산정했으나 실제 BE팀
  용도(Session/Cache/PubSub)   용도 및 필요 여부 미확정           

  AI팀 외부 API 사용 여부·비용 AI팀 요구사항 문서상 "외부 API     AI팀
                               미사용" 명시, 최신 상황 재확인     
                               필요                               

  OpenSearch 사용 여부·방식    관리형 OpenSearch 비용은           BE/FE팀
                               산정했으나 실제 검색 기능 적용     
                               여부 미확정                        

  BE/AI Pod Resource Spec      CPU Worker 기반으로 변경됨. 최종   BE/AI팀 / 인프라팀
                               `requests/limits` 확인 후          
                               `t3.large × N` 수용 가능 여부 검증 
                               필요                               

  도메인 결제 주체·카드        미정                               도메인 네임 활용
                                                                  여부부터 타 파트와
                                                                  논의할 것. 결제는
                                                                  상우님이 인프라 포함
                                                                  일괄 결제
