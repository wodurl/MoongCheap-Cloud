...
-   AWS Calculator 월간 상시 운영 비용(730h): **\$730.36/month**
...
현재 구성과 운영시간을 준수할 경우 **약 \$89의 비용 Buffer**를 확보할 수
있을 것으로 예상한다.
...
발생하는 추가 Node-hours는 \$88.91 Buffer 내에서 우선 수용**한다.
...
`$463.09`\*\*로 산정하며, 프로젝트 진행 중에는 AWS Cost Explorer의 실제
...
월 비용 `$730.36` 전체에 `368/730`을 적용하면 안 된다.
...
따라서 `$730.36 × 368/730`은 실제 프로젝트 비용이 아니며, **서비스별
...
사용하며, Amazon OpenSearch Service 도입으로 확정한다.
...
Redis는 **Amazon ElastiCache 도입으로 확정**한다. 네이밍 규약서 3.8절과
아키텍처 설계서 6.4절에서도 확정 구성 요소로 관리한다.

다만 예산이 부족한 경우에는 EKS 내부 Redis Pod 자체 호스팅으로 전환하는
방안을 비용 절감 대안으로 비교한다(비용 관리 Runbook 8장 권장 운영 규칙
참고).
